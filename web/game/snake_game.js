...
class SnakeGame {
  ...
  setOverlay(visible, title = "", text = "") {
    ...
    this.overlay.toggleAttribute("inert", !visible);
  ...
  getObservationTensor() {
    return computeObservationTensor(featureStateFromGame(this));
  }

  getTinyFeatures() {
    return computeTinyFeatures(featureStateFromGame(this));
  }

  getLocalPatch(patchSize = 11) {
    return computeLocalPatch(featureStateFromGame(this), patchSize);
  }
  ...
  createAgentAPI() {
    return Object.freeze({
      ...
      getLocalPatch: (patchSize) => this.getLocalPatch(patchSize),
