...
- 如果你只关心 Python 训练入口，请优先看 `README.md` 和 `train_config.py`
...
如果你使用的是项目内置 Python 训练栈，还可以配合这两个脚本：
...
它会对当前 `train_config.py` 做短时 benchmark，并输出保守 / 中等 / 偏长三档训练时长估算。
...
打开 `index.html` 后，使用页面新增的 `AI 接管` 面板：
...
2. 状态通过 HTTP 发给本地 `serve_model_inference.py`
