...
- **浏览器即玩**：`web/index.html` 无依赖直接运行，支持键盘/触屏
- **Double DQN 训练**：三种网络结构（`small_cnn` / `adaptive_cnn` / `hybrid`），支持课程学习、随机地图、并行 rollout
- **断点续训**：完整恢复训练状态（优化器 + 回放池 + 进度）或仅热加载权重
- **Web 控制台**：训练管理、运行记录、TensorBoard 监控、模型演示一体化（`snake-webui`）
- **规则对齐**：`web/game.js` 与 `snake_rl/env.py` 动作/观测/奖励语义完全对应，模型可直接用于浏览器推理
- **工程化配置**：JSON Schema 约束的 `custom_train_config.json`，编辑器即时校验
...
flowchart LR
    ...
    B --> C["runs/<run_name>/"]
    C --> D["snake-rl eval"]
    C --> E["snake-rl monitor（TensorBoard）"]
    C --> F["snake-rl serve-model"]
    F --> G["web/index.html 或 /play/ 中演示 AI"]
```
...
| 类型 | 输入 | 特点 | 适用 |
| --- | --- | --- | --- |
| `small_cnn` | 固定 `[H, W, 9]` | 简单快速 | 固定尺寸地图、入门验证 |
| `adaptive_cnn` | 可变 `[H, W, 9]`，GAP | 通用，支持 curriculum/random_board | 多数场景推荐 |
| `hybrid` | 局部 patch + 10 维全局特征 | 最强泛化，计算略重 | 跨尺寸泛化、长期训练 |
...
snake_rl/
  ...
  form_field_tips.py   ← Web 表单字段元数据
  process_supervisor.py ← 子进程终止（跨平台）
web/
  index.html           ← 独立贪吃蛇游戏页
  app.html             ← Web 控制台（Vue 3 CDN）
  game.js              ← 游戏引擎 + RL Agent API
  style.css            ← 深色主题样式
tests/
  test_env_and_history.py    ← 环境奖励与历史恢复
  test_replay_and_config.py  ← 回放池与配置反序列化
  test_cli_smoke.py          ← CLI 基本行为与错误路径
  test_agent_hybrid_schema.py ← hybrid checkpoint 版本校验
  test_run_context.py        ← run 上下文解析
docs/
  python-training.md         ← 训练/评估/监控完整指南
  custom-train-config.md     ← 配置字段调参手册
  custom-train-config.html   ← 浏览器参数手册（Web 控制台内嵌）
  browser-agent-api.md       ← 网页侧 RL API 说明
  js-rule-mapping.md         ← JS 与 Python 规则对齐文档
custom_train_config.json     ← 默认训练配置（带 Schema 校验）
custom_train_config.schema.json ← JSON Schema（VS Code / WebUI 即时校验）
```
...
- [`docs/custom-train-config.html`](docs/custom-train-config.html)：浏览器可读参数手册
...
- CLI 基本行为与错误路径（`test_cli_smoke.py`）
- 环境距离塑形奖励、episode 历史快照恢复（`test_env_and_history.py`）
- 回放池序列化、配置反序列化与 custom 路径逻辑（`test_replay_and_config.py`）
- run 上下文与 checkpoint 路径解析（`test_run_context.py`）
- hybrid checkpoint 特征 schema 版本校验（`test_agent_hybrid_schema.py`）
